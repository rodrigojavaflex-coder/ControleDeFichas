/**
 * SVG inline no estilo Lucide/Feather (stroke, currentColor, 24×24).
 *
 * Sem import de `lucide` em runtime: o pacote lucide declarou `module` apontando
 * para `dist/esm/lucide.js` (inexistente), o que faz o esbuild/vite do `ng serve` falhar.
 * Os valores abaixo foram gerados a partir do bundle CJS (Lucide 0.563.0) pelo script
 * `scripts/extract-lucide-svgs.mjs` — execute-o após atualizar `lucide` no dev.
 */

export const NAV_LOGOUT_ICON_SVG_HTML =
...
const RAW: Record<string, string> = {
  ...
  'feather-user-check':
    '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m16 11 2 2 4-4" /><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /></svg>',
  'feather-user-plus':
...
const ALIASES: Record<string, string> = {
  'feather-settings': 'feather-cog',
};

export function getNavigationMenuIconHtml(iconKey?: string | null): string {
  const key = (iconKey ?? '').trim();
  const resolved = ALIASES[key] ?? key;
  return RAW[resolved] ?? RAW['feather-file-text'];
}
