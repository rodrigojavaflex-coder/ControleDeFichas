...
@Component({
  selector: 'app-folha-cargos-page',
  standalone: true,
...
export class FolhaCargosPage implements OnInit {
  ...
  ngOnInit(): void {
    this.pageCtx.setContext({
      title: 'Cargos da folha',
      description: 'Cadastro de cargos utilizados no vínculo com funcionários.',
    });
    this.reload();
  }
  ...
  pode(): { r: boolean; c: boolean; u: boolean; d: boolean; a: boolean } {
    return {
      r: this.auth.hasPermission(Permission.FOLHA_CARGO_READ),
      c: this.auth.hasPermission(Permission.FOLHA_CARGO_CREATE),
      u: this.auth.hasPermission(Permission.FOLHA_CARGO_UPDATE),
      d: this.auth.hasPermission(Permission.FOLHA_CARGO_DELETE),
      a: this.auth.hasPermission(Permission.FOLHA_CARGO_AUDIT),
    };
  }

  irNovo(): void {
    void this.router.navigate(['/folha/cargos/new']);
  }

  irEditar(v: FolhaCadastroSimples): void {
    void this.router.navigate(['/folha/cargos/edit', v.id]);
  }

  reload(): void {
    if (!this.pode().r) return;
    this.carregando = true;
    this.folha.listarCargos().subscribe({
      next: (data) => {
        this.lista = data;
      ...
      error: () => {
        this.carregando = false;
        this.errors.show('Erro ao carregar cargos.', 'Folha');
      },
    });
  }
  ...
  alternarAtivo(v: FolhaCadastroSimples): void {
    ...
    this.folha.patchCargo(v.id, { ativo: !v.ativo }).subscribe({
      next: () => this.reload(),
      error: (e) =>
        this.errors.show(e?.error?.message ?? 'Erro ao atualizar.', 'Cargo'),
    });
  }

  excluir(v: FolhaCadastroSimples): void {
    if (!this.pode().d) return;
    if (!confirm(`Excluir cargo "${v.descricao}"?`)) return;
    this.folha.deleteCargo(v.id).subscribe({
      next: () => this.reload(),
      error: (e) =>
        this.errors.show(
