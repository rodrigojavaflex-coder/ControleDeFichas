...
export class FolhaVerbasPage implements OnInit {
  ...
  ngOnInit(): void {
    this.pageCtx.setContext({
      title: 'Lista de verbas',
      description: 'Eventos de receita e despesa. Novo cadastro abre formulário próprio.',
    });
    this.reload();
  }
  ...
  reload(): void {
    ...
    this.folha.listarVerbas().subscribe({
      ...
      error: () => {
        this.carregando = false;
        this.errors.show('Erro ao carregar verbas.', 'Folha');
      },
    });
  }
  ...
  alternarAtivo(v: FolhaVerba): void {
    ...
    this.folha.patchVerba(v.id, { ativo: !v.ativo }).subscribe({
      next: () => this.reload(),
      error: (e) => this.errors.show(e?.error?.message ?? 'Erro ao atualizar.', 'Verba'),
    });
  }

  excluir(v: FolhaVerba): void {
    if (!this.pode().d) return;
    if (!confirm(`Excluir verba "${v.descricao}"?`)) return;
    this.folha.deleteVerba(v.id).subscribe({
      next: () => this.reload(),
      error: (e) => this.errors.show(e?.error?.message ?? 'Não foi possível excluir.', 'Verba'),
    });
  }
