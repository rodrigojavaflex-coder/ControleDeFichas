...
@Component({
  selector: 'app-fechamento-vendas-list',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class FechamentoVendasListComponent extends BaseListComponent<Venda> implements OnDestroy {
  ...
  showConfirmAtualizarValorCompraModal = false;
  atualizarComValorCliente = false;
  ...
  override ngOnInit(): void {
    ...
    this.pageContextService.setContext({
      ...
      description: 'Feche ou cancele fechamentos das vendas selecionadas, com impressão e exportação.'
    });
    super.ngOnInit();
  }
  ...
  onPageChange(page: number): void {
    if (this.atualizarValorCompraLoading) {
      return;
    }
  ...
  imprimirFechamentoAtual(): void {
    ...
    const totalValorCompra = podeVerValorCompra
      ...
      : 0;
    const totalValorPagoCompra = podeVerValorCompra
      ? vendasParaImprimir.reduce((total, venda) => total + (venda.valorPago || 0), 0)
      : 0;
        ...
        return `
          <section class="unit-section">
            ...
            <div class="unit-total-row">
              ...
              ${podeVerValorCompra ? `<span><strong>Total Fechamento:</strong> ${this.formatCurrency(unidadeValorPagoCompra)}</span>` : ''}
            </div>
          </section>`;
      })
      .join('');
  ...
  exportarSelecionadasParaExcel(): void {
    ...
    const planilha = vendas.map(venda => {
      const linha: Record<string, string | number> = {
        ...
        Atualização: this.getTipoAtualizacaoLabel(venda.tipoAtualizacao),
        Observação: venda.observacao || ''
      };

      if (podeVerValorCompra) {
        // Inserir as colunas de compra na ordem correta
        linha['Valor Compra'] = venda.valorCompra || 0;
        linha['Valor Fechamento'] = venda.valorPago || 0;
      }
  ...
  getTipoAtualizacaoLabel(tipoAtualizacao?: TipoAtualizacao): string {
    if (!tipoAtualizacao) {
      return '-';
    }
    const labels = {
    ...
    return labels[tipoAtualizacao] || tipoAtualizacao;
  }
  ...
  fecharVendasSelecionadas(): void {
    ...
    this.vendaService.fecharVendasEmMassa(vendaIds).subscribe({
      next: (resultado) => {
        ...
        if (resultado.falhas?.length) {
          // Agrupar falhas por motivo
          const falhasPorMotivo = new Map<string, string[]>();
          
          for (const falha of resultado.falhas) {
            const motivo = falha.motivo || 'Não pôde ser fechada.';
            const protocolo = falha.protocolo || falha.id;
            
            if (!falhasPorMotivo.has(motivo)) {
              falhasPorMotivo.set(motivo, []);
            }
            falhasPorMotivo.get(motivo)!.push(protocolo);
          }
          
          // Formatar mensagem agrupada
          const detalhes: string[] = [];
          for (const [motivo, protocolos] of falhasPorMotivo.entries()) {
            const quantidade = protocolos.length;
            const maxProtocolos = 20; // Limitar a 20 protocolos por linha
            const protocolosParaMostrar = protocolos.slice(0, maxProtocolos);
            const protocolosRestantes = quantidade - maxProtocolos;
            
            detalhes.push(`\n${motivo} (${quantidade} venda${quantidade > 1 ? 's' : ''})`);
            
            if (protocolosRestantes > 0) {
              detalhes.push(`  ${protocolosParaMostrar.join(', ')}... (+${protocolosRestantes} mais)`);
            } else {
              detalhes.push(`  ${protocolosParaMostrar.join(', ')}`);
            }
          }
          
          this.errorModalService.show(`Algumas vendas não foram fechadas:${detalhes.join('\n')}`, 'Aviso');
        } else {
    ...
    this.vendaService.cancelarFechamentosEmMassa(vendaIds).subscribe({
      next: (resultado) => {
        ...
        if (resultado.falhas?.length) {
          ...
          for (const falha of resultado.falhas) {
            const motivo = falha.motivo || 'Não pôde ter o fechamento cancelado.';
            const protocolo = falha.protocolo || falha.id;
          ...
          this.errorModalService.show(`Algumas vendas não tiveram o fechamento cancelado:${detalhes.join('\n')}`, 'Aviso');
        } else {
  ...
  atualizarValorCompraSelecionadas(): void {
    if (!this.canViewValorCompra()) {
      return;
    }

    if (this.selectedVendas.size === 0) {
      this.errorModalService.show('Selecione pelo menos uma venda para atualizar o valor de compra.', 'Atenção');
      return;
    }

    // Validação básica apenas para vendas sem origem (não bloqueia protocolos inválidos)
    const vendas = this.getSelectedVendaModels();
    const vendasSemOrigem = vendas.filter(v => !v.origem);
    if (vendasSemOrigem.length > 0) {
      const protocolos = vendasSemOrigem.map(v => v.protocolo || v.id).join(', ');
      this.errorModalService.show(`As seguintes vendas não possuem origem definida: ${protocolos}`, 'Atenção');
      return;
    }

    this.atualizarComValorCliente = false;
    this.showConfirmAtualizarValorCompraModal = true;
  }

  confirmarAtualizarValorCompra(): void {
    this.showConfirmAtualizarValorCompraModal = false;
    const vendaIds = this.getSelectedVendaModels().map(venda => venda.id);
    this.atualizarValorCompraLoading = true;

    this.vendaService.atualizarValorCompraEmMassa(vendaIds, this.atualizarComValorCliente).subscribe({
      next: (resultado) => {
        this.atualizarValorCompraLoading = false;
        this.selectedVendas.clear();
        this.loadItems();

        const totalProcessadas = (resultado.sucesso?.length || 0) + (resultado.falhas?.length || 0);
        const totalSucesso = resultado.sucesso?.length || 0;
        const totalFalhas = resultado.falhas?.length || 0;
        ...
        if (resultado.falhas?.length) {
          ...
          for (const falha of resultado.falhas) {
            const motivo = falha.motivo || 'Não pôde ter o valor de compra atualizado.';
            const protocolo = falha.protocolo || falha.id;
          ...
          const mensagem = `Processamento concluído:\n` +
            `• Total processadas: ${totalProcessadas}\n` +
            `• Atualizadas com sucesso: ${totalSucesso}\n` +
            `• Com avisos: ${totalFalhas}` +
            `\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
            `Detalhes dos avisos:${detalhes.join('\n')}`;
          
          this.errorModalService.show(mensagem, 'Aviso');
        } else {
          const mensagem = `Processamento concluído:\n` +
            `• Total processadas: ${totalProcessadas}\n` +
            `• Atualizadas com sucesso: ${totalSucesso}\n` +
            `• Com avisos: 0`;
          this.errorModalService.show(mensagem, 'Sucesso');
        }
      },
      error: (error) => {
        this.atualizarValorCompraLoading = false;
        let message = 'Erro ao atualizar valor de compra das vendas';
  ...
  cancelarAtualizarValorCompra(): void {
    this.showConfirmAtualizarValorCompraModal = false;
    this.atualizarComValorCliente = false;
  }
