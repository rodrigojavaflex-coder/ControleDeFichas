...
- **Objetivo oficial:** o PDF Fórmula Certa (visitação) é a **base do cálculo da fórmula** — eixo **DTEFE**, exclusão de cortesia `C`, tipo `N` sem fórmula fora, rateio/teto `PRCOBR` e **crédito por série (`FC12100.SERIER`)**. O recorte da **comissão oficial** é a **soma das unidades marcadas** na Configuração Metas (RN-VIS-011) para aquele representante — no card, iguais ao PDF por filial (ex. Nerópolis do 99 inclui Roberta 23287). Sem unidade extra marcada, o recorte continua só a filial do cadastro/filtro (Loja). No ERP, 9999/99 e 9999/199 rodam no **mesmo banco** e misturam filiais; o mesmo CRM pode sair nos **dois** PDFs. O NEST **mostra o conflito** na meta e **não bloqueia** o save; o mesmo real pode entrar na comissão dos dois cadastros até o painel ser corrigido no ERP.
- **Recebido — universo e data:** itens ERP de requisição (`caixa_itens_erp`, `tipo_item = REQUISICAO`). **Eixo de data:** se houver paga, **`data_pagamento` (FC17000.DTEFE / DT PAGTO do PDF)**; se a paga não veio na sync, `data_operacao` do cupom. Cupom no período com DTEFE fora **não entra** (ex. 97987, paga 03/08). Cupom fora com DTEFE no período **entra** (ex. 96677, parcela 30/06 paga em 03/07).
- **Recebido — valor da requisição:** `caixa_requisicoes_pagas.valor_pago_requisicao` (FC17000.VRLIQ **pago**); se não houver linha paga, usa `caixa_itens_erp.valor_liquido_item` (FC31110.VRLIQ). A paga é ligada por **unidade + número da requisição** (preferência pelo mesmo cupom; se o cupom da parcela for outro, usa a paga da req). Se o **item do cupom** for menor que o `valor_pago_requisicao` (parcelamento `TPRQU=S` em dois cupons, ex. 97494), o prescritor recebe só a **parcela do cupom**. **Cortesia** (`tipo_requisicao = C` / FC17000.TPRQU, ou item com `valor_liquido_linha = 0` ainda sem tipo) **não** entra. **Tipo N** (`TPRQU=N`) **sem** fórmula em `FC12100` (ex. 97785 + taxa de caixa) **não** entra. **Não** usar `valor_liquido_linha` (total do cupom). `caixa_requisicoes_pagas.valor_formulas` = soma `FC12100.PRCOBR` × `LEAST(1, VRLIQ/VRRQU)` — total da req para caixa detalhado e comercial (ex. 97510 → **281,00**; tipo N com extra, PDF **155,50**, não 415).
- **Recebido — crédito por fórmula (`SERIER`):** o prescritor recebe a soma de `caixa_requisicao_formula.valor_rateado` das **séries com o CRM dele** (`valor_rateado` = `PRCOBR` × o mesmo `LEAST(1, VRLIQ/VRRQU)` da paga). Requisição com vários médicos (ex. **98112**: Cristian séries 2–3, Matheus série 1) **parte** o valor; **proibido** creditar o VRLIQ/teto inteiro na série `0`. Sem linhas de fórmula (paga anterior à sync da RN-CXA-003), fallback legado: CRM da paga e teto `valor_formulas` da req. Detalhe/impressão analítica listam **série**, não só a requisição. Prescritor da série; se a série não tiver CRM, fallback só `FC17000`. **Não** usar orçamento (`nrorc`) para creditar prescritor.
...
- **Ordem:** não fechar competência em produção sem `caixa_requisicao_formula` populado no período (RN-VIS-008 / RN-CXA-003); senão o retrato congela o crédito da série `0`.
...
- **Incidência:** **Vendas próprias** (recebido do vendedor, regras RN-VIS-008 nos manipulados) ou **Vendas da loja** (volume do caixa confirmado da unidade naquela base, mesmo do card Total — RN-COM-003).
...
- **Proteção do complemento:** se o snapshot de **requisições pagas** vier **vazio** **ou incompleto** (menos da metade das requisições distintas dos itens `REQUISICAO` do mesmo segmento), a importação **aborta** — **não** grava pagamentos, itens nem pagas. Mensagem: detalhe incompleto do ERP; atualizar vendas de novo antes de fechar. Evita o dia ficar com cupom/total conferido e **sem** `caixa_requisicoes_pagas` (visitação/comercial somem da grade). **Fechar o caixa** do dia também recusa se existir requisição no cupom sem linha paga.
- **`caixa_requisicoes_pagas`:** orçamento (`NRORC`), qtd/valor de fórmulas e prescritor **agregado** em `FC12100` por requisição (prioriza série `0`, depois outras fórmulas e requisição-fonte `NRRQUFON`) — usado no caixa detalhado e no comercial. Considera apenas `NRORC > 0`. Se o CRM não vier da fórmula, usa `FC17000.PFCRM/UFCRM/NRCRM`. A busca inclui `FC17000.dtefe` **ou** requisição presente no cupom do dia (`FC31200.dtope`). Grava `tipo_requisicao` (`TPRQU`) e `valor_formulas` = soma `FC12100.PRCOBR` × `LEAST(1, VRLIQ/VRRQU)`. Vendedor: `FC17200` (`TRIM(TPTAR)='R'`) e, se vazio, `FC17000.CDFUN`. **Não altera** totais de `caixa_pagamentos_erp`.
...
- **Pré-condição ao confirmar:** não pode haver item `REQUISICAO` do dia sem `caixa_requisicoes_pagas` (mesmo critério do complemento na RN-CXA-003). Os totais de forma de pagamento podem bater com o ERP e o detalhe da requisição ainda estar ausente — nesse caso **Atualizar Vendas** de novo, não fechar.
