...
- Cards abaixo dos filtros, no mesmo padrão: **TOTAL {unidade}** primeiro (ex. `TOTAL INHUMAS`) e em seguida **um card por representante** (não exibe o card **Sem representante**; as linhas da grade sem vínculo continuam visíveis). Faixa operacional nos cards de representante: **Recebido**, **Rejeitado** e **Representatividade**. O card Total exibe **Loja** e **Outras unidades** (recebido e rejeitado em cada bloco); **não** exibe Representatividade, **nem** Meta / % da meta / Projeção.
- **Recebido do Total:** o card **TOTAL {unidade}** separa **Loja** e **Outras unidades**. **Loja — Recebido:** soma de **todas** as requisições do **caixa da unidade** na competência (RN-VIS-008: `REQUISICAO`, DTEFE, sem cortesia C, tipo N sem fórmula fora, teto de fórmula), **sem** exigir CRM nem representante. **Loja — Rejeitado:** orçamentos `REJEITADO` da mesma unidade. **Outras unidades:** recebido e rejeitado de **indicação** em outras filiais (médico na carteira da unidade do filtro e **não** no painel da unidade do movimento — RN-VIS-008); entram só neste bloco do Total e nos cards dos representantes, **não** no recebido da loja. Sem unidade (`ALL`): só o bloco Loja, com caixa de **todas** as unidades do escopo. Independente dos filtros de representante/médico/No Painel.
- **Representatividade** (cards de representante): recebido do representante ÷ **recebido do caixa da unidade** (Total, incluindo reqs sem prescritor; não o total atribuído da grade).
