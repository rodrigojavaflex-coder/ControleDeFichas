...
- **Objetivo oficial:** o PDF analítico Fórmula Certa (visitação) é a **base de confiança do gestor** e o recorte que a tela deve reproduzir. Fonte no NEST: tabelas do **Atualizar Vendas** — `caixa_requisicoes_pagas` (`FC17000`, eixo **DTEFE / DT PAGTO**) + `caixa_requisicao_formula` (uma linha por **série `FC12100.SERIER`**). **Não** parte de `caixa_itens_erp` nem usa órfão por `data_operacao`. Exclusão de cortesia `C`, tipo `N` sem fórmula fora, crédito `valor_rateado` (`PRCOBR` × `LEAST(1, VRLIQ/VRRQU)`). O recorte da **comissão oficial** é a **soma das unidades marcadas** na Configuração Metas (RN-VIS-011) para aquele representante — no card, iguais ao PDF por filial (ex. Nerópolis do 99 inclui Roberta 23287). Sem unidade extra marcada, o recorte continua só a filial do cadastro/filtro (Loja). No ERP, 9999/99 e 9999/199 rodam no **mesmo banco** e misturam filiais; o mesmo CRM pode sair nos **dois** PDFs. O NEST **mostra o conflito** na meta e **não bloqueia** o save; o mesmo real pode entrar na comissão dos dois cadastros até o painel ser corrigido no ERP.
- **Recebido — universo e data:** **`caixa_requisicoes_pagas`** com **`data_pagamento` (FC17000.DTEFE)** no intervalo da competência. Cupom no período com DTEFE fora **não entra** (ex. 97987, paga 03/08). Cupom fora com DTEFE no período **entra** (ex. 96677, parcela 30/06 paga em 03/07). Requisição no cupom **sem paga** **não entra** (o complemento da sync não grava item órfão — RN-CXA-003). **Saldo em aberto (`FC17000.VRSDO` > 0)** **não entra** — o PDF “Receitas Recebidas” só traz a receita quitada (ex. 99383/99377/99423 Matheus, 99306 Luiza). Tipo `S` **quitado** (`VRSDO = 0`, ex. Hugo **98842**, Kassiana **99169**) **entra**.
- **Recebido — valor da requisição (card TOTAL / Loja):** `valor_pago_requisicao` (FC17000.VRLIQ **pago**), com teto `valor_formulas` quando o pago supera as fórmulas. **Não** corta pelo item do cupom (parcelamento em dois cupons entra pelo DTEFE de cada paga). **Cortesia** (`tipo_requisicao = C`) **não** entra. **Tipo N** sem fórmula em `FC12100` **não** entra. **Não** usar `valor_liquido_linha` (total do cupom). `valor_formulas` = soma `FC12100.PRCOBR` × `LEAST(1, VRLIQ/VRRQU)` — total da req para caixa detalhado e comercial (ex. 97510 → **281,00**; tipo N com extra, PDF **155,50**, não 415). O **Recebido Loja** do card Total é a loja inteira (RN-VIS-010), **não** o PDF sintético de um visitador.
- **Recebido — crédito por fórmula (`SERIER`):** o prescritor recebe a soma de `caixa_requisicao_formula.valor_rateado` das **séries com o CRM dele**. Requisição com vários médicos (ex. **98112**: Cristian séries 2–3, Matheus série 1) **parte** o valor; **proibido** creditar o VRLIQ/teto inteiro na série `0`. Sem linhas de fórmula (paga anterior à sync da RN-CXA-003), fallback legado: CRM da paga e teto `valor_formulas` da req. Detalhe/impressão analítica listam **série**, não só a requisição. Prescritor da série; se a série não tiver CRM, fallback só `FC17000`. **Não** usar orçamento (`nrorc`) para creditar prescritor.
- **Comissão oficial = unidades configuradas:** o recebido que entra em **% meta, projeção, faixa e comissão** é a **soma das unidades** marcadas na Configuração Metas (RN-VIS-011) para aquele representante. A unidade do cadastro entra sempre. Sem configuração extra, o recorte continua só a filial do filtro (Loja). 99 (Inhumas) e 199 (Nerópolis) são **dois representantes** (RN-VIS-005); o freeze trava a **unidade inteira** (RN-VIS-013), não um `cdfun`. CRM nos dois painéis: o NEST **mostra o conflito** na meta e **não bloqueia** o save; o mesmo real pode aparecer na comissão dos dois cadastros até o ERP/painel ser corrigido. **Uberaba** (outra base ERP) só entra se estiver marcada nas unidades de comissão.
...
- **Indicação / visibilidade:** movimento em **outra filial** só entra na **grade** (e no card) se a filial estiver nas **unidades de comissão** do representante (RN-VIS-011). Sem marcação, a req de outra unidade **não aparece** na grade. Com a filial marcada, o valor entra no total, % meta, projeção e comissão (mesmo com CRM também no painel local dessa filial — ex. Roberta 23287 em Nerópolis no card do 99).
- **Filial no PDF = loja da venda (`cdfil`), não o painel da outra unidade.** 9999/99 (Inhumas) e 9999/199 (Nerópolis) são **dois** visitadores. No card: bloco da **outra** filial = vendas do **mesmo painel** na **outra loja** (99 em `cdfil` 4; 199 em `cdfil` 2). **Não** misturar painel 199 nas vendas de Inhumas do 99, nem o contrário.
- **Aceite consolidado (sintético Fórmula Certa × card do representante):** agosto/2026 9999/99 — Inhumas **R$ 99.605,94** · Nerópolis **R$ 3.899,74** · geral **R$ 103.505,68**; 9999/199 — Inhumas **R$ 4.501,73** · Nerópolis **R$ 18.193,59** · geral **R$ 22.695,32**. 01–09/09/2026 9999/99 — Inhumas **R$ 31.325,75** · Nerópolis **R$ 1.461,60** · geral **R$ 32.787,35**. Cruzamento de filial **fecha exatamente**. Folga residual na **loja do próprio** painel (rateio `valor_rateado` × VALOR do PDF, ordem de R$ 1–200) **não** abre recorte novo. Card **TOTAL Loja** continua a loja inteira (RN-VIS-010), **não** o PDF do visitador.
- Registro **sem CRM ou UF na série (`FC12100`) e na paga (`FC17000`)** não entra no crédito da grade nem no card do representante (ex. req **98656**). **Não** usar orçamento (`nrorc`) para creditar prescritor. Continua no **Recebido Loja** do card Total (RN-VIS-010), que não exige prescritor.
- Endpoints: **`GET /visitacao/acompanhamento`** (grid paginada + totais; query **`ano`** + **`mes`**), **`GET /visitacao/acompanhamento/detalhe`** (intervalo derivado da competência) e **`GET /visitacao/acompanhamento/opcoes-filtro`**. Fechar/reabrir: **RN-VIS-013**.
...
- Filtros **inline** no mesmo padrão do acompanhamento comercial: **Unidade**, **competência** (mês/ano com setas para o mês anterior e o próximo), **Representante** (combo), **Médico** (texto, busca parcial por nome) e **No Painel**. **Não** há filtro de CRM/UF. A listagem **só carrega depois que a unidade está informada** (usuário com `unidade` no cadastro já entra com o filtro fixo).
- Filtro **Unidade** (rótulo na tela): **painel + caixa daquela filial**. A **comissão oficial** e a **grade de outras filiais** seguem as **unidades de comissão** do representante (RN-VIS-008 / RN-VIS-011). Usuário com `unidade` cadastrada tem o filtro fixo na própria unidade.
- Usuário **sem** unidade: informa a unidade no filtro; uma linha por evento (unidade = onde recebeu/rejeitou), sem duplicar valor.
...
- Filtro **Representante** (combo) restringe à carteira do funcionário vinculado (par filial/código). Movimento em outra unidade na grade só se a filial estiver marcada nas unidades de comissão (RN-VIS-008); o recebido oficial continua a soma dessas unidades.
- Filtro de **médico** por **nome** (parcial, `ILIKE`). Endpoint de listagem: **`GET /visitacao/acompanhamento`**.
- Período: competência **mês + ano** (2026–2033), padrão o mês corrente. Setas à esquerda/direita avançam ou recuam um mês (dezembro → janeiro do ano seguinte; janeiro/2026 e dezembro/2033 desabilitam a seta correspondente). O backend deriva o intervalo civil (`YYYY-MM-01` até o último dia). Ordenação padrão: **Recebido** do maior para o menor. Clique no cabeçalho da coluna reordena a grid (paginação no servidor).
...
- Escopo de carteira do usuário conforme **RN-VIS-003** / **RN-007**. Outras filiais na grade só com unidade marcada em comissão (RN-VIS-008 / RN-VIS-011).

### RN-VIS-010 — Cards, detalhe e impressão

- Cards abaixo dos filtros, no mesmo padrão: **TOTAL {unidade}** primeiro (ex. `TOTAL INHUMAS`) e em seguida **um card por representante** (não exibe o card **Sem representante**; as linhas da grade sem vínculo continuam visíveis). Faixa operacional nos cards de representante: recebido **nas unidades de comissão** (RN-VIS-011; rótulo = nome da filial), **Total** (soma dessas unidades), **Rejeitado** e **Representatividade**. Meta, faixa, comissão, % meta e projeção usam o **Total**. O card Total exibe **Loja**, um bloco por **outra filial marcada** (se houver) e o **Total**; **não** exibe Representatividade, **nem** Meta / % da meta / Projeção.
- **Recebido do Total:** o card **TOTAL {unidade}** separa **Loja** e as **outras filiais marcadas** na Configuração Metas (RN-VIS-011; união das unidades de comissão dos representantes da unidade do filtro). **Loja — Recebido:** soma de **todas** as pagas da unidade no eixo **DTEFE** (RN-VIS-008), DISTINCT requisição/cupom, **sem** exigir CRM nem representante — **não** é o PDF sintético de um visitador. **Loja — Rejeitado:** orçamentos `REJEITADO` da mesma unidade. **Outras filiais:** um bloco por unidade marcada (recebido e rejeitado da carteira da loja naquela filial) e um **Total** (Loja + outras). Sem nenhuma unidade extra marcada, o card **não** exibe outras filiais. Sem unidade (`ALL`): só o bloco Loja. Independente dos filtros de representante/médico/No Painel. Não entram em representatividade, meta, faixa nem comissão. **Entram no retrato** do fechamento (RN-VIS-013).
- **Representatividade** (cards de representante): recebido **oficial** do representante (soma das unidades de comissão) ÷ **recebido do caixa da unidade do filtro** (Loja, incluindo reqs sem prescritor).
- No cabeçalho dos cards de **representante**: **com movimento** (linhas da grade no filtro), **ativos no painel** (carteira atual em `painel_medicos_representantes`) e **fora do atendimento** (ativos sem recebido/rejeitado na competência **no recorte das unidades de comissão** da loja — filial do filtro + filiais extras marcadas na Configuração Metas; RN-VIS-011). Movimento só em filial **não** marcada não tira o médico de “fora”. O card **Total** não exibe essas contagens.
- Faixa de desempenho **só nos cards de representante** (mesma competência do filtro): **Meta** (`visitacao_meta_representante`), **% Meta** (recebido ÷ meta) e, se a competência **ainda estiver aberta** (hoje em America/Sao_Paulo dentro do mês), **Projeção** = `(recebido ÷ dias realizados) × dias úteis do mês` e **% da projeção** (projeção ÷ meta). **Dias realizados** = dias úteis da competência com data **≤ último caixa CONFIRMADO** da unidade (mesmo critério Fechado/Bloqueado da RN-CXA-009): sábado = **0,45**; feriado/domingo = 0. Se o último confirmado for depois do mês, conta o mês inteiro. Sem confirmação, 0 e não projeta. Sem meta cadastrada: **Sem meta** (não 0%).
- **Faixa e comissão** no card do representante somente com **`visitacao-acompanhamento:comissao`**: faixa vigente em `visitacao_comissao_faixa` pelo **% da meta**; valor = `% da faixa × recebido oficial` (soma das unidades de comissão). Com mês **aberto** (RN-VIS-013): também **Faixa proj.** e **Comissão proj.** pelo **% da projeção**, na **mesma linha**. Competência **fechada:** sem projeção; faixa/comissão vêm do retrato. Sem permissão, os campos não vêm na API.
- **Quadro de legenda** à direita da barra de filtros: competência (**mês ano**, ex. `Agosto 2026`) e **status** (Aberto/Fechado) **na mesma linha**; na linha seguinte, **dias úteis** e **dias realizados** juntos (ex.: `21 dias úteis · 18 dias realizados`). Calendário pela unidade do filtro.
- Clique na linha ou no link **Detalhes** (coluna **Ações**, após Rejeitado) abre o detalhe: requisições pagas do caixa (com **série** quando houver `caixa_requisicao_formula`) e orçamentos rejeitados **da unidade do movimento**; permitido se a unidade for a do usuário **ou** o CRM estiver na carteira dele **e** a filial do movimento estiver nas **unidades de comissão** desse representante. **Valor Pago** do recebido é o da série/requisição (RN-VIS-008), não o total do cupom. Valores de **Recebido** em verde e **Rejeitado** em vermelho na grid.
- Impressão da listagem abre modal de opções: **Analítico** (detalhado, padrão) ou **Sintético** (resumo por representante/unidade); movimentos **Todos** (padrão), **Recebidos** ou **Rejeitados**. Analítico agrupa **Representante → Unidade** sem cards no topo; o rodapé de cada tabela alinha quantidade de médicos e totais nas colunas Recebido/Rejeitado. Impressão **oficial de comissão** (sintético fechado) usa o **recebido oficial** (soma das unidades de comissão).
...
- Coluna **Unidades comissão** (entre Meta e Ações): checkboxes das filiais que entram na **grade**, no card, no total, na % meta, na projeção e na comissão. Só **marca/desmarca em Alterar ou Incluir**. A unidade do cadastro do representante é **obrigatória** (não desmarca). Persistência em `visitacao_representante_unidade_comissao` (**por funcionário**, vale para todos os meses). Sem linhas, o recorte é só a unidade do cadastro.
- Ao marcar/desmarcar unidades, a API **valida o painel** entre as filiais escolhidas: mesmo CRM+UF em **mais de uma** unidade = conflito. **Não bloqueia** o save. Coluna **Ações**: **Alerta (n)** abre o modal (médico, CRM, UF, unidades, representantes do ERP); **Reavaliar painel** recalcula e **some o alerta** se não houver mais conflito (após sync). No modal: **Imprimir** no padrão dos relatórios (logo, título centralizado, rodapé “Gerado em … por …”).
- Ação **Copiar mês anterior**: disponível só com um mês específico selecionado; replica as metas da competência anterior para os representantes ainda vinculados; valores já existentes no destino são substituídos. **Não** copia as unidades de comissão (são do cadastro do representante).
...
### RN-VIS-013 — Fechamento mensal da visitação (retrato)

- **Chave:** competência **`ano + mês + unidade`**. Um fechamento trava **todos** os representantes da loja (99 e 199 são cadastros distintos; Inhumas fecha sem fechar Nerópolis). **Não** há freeze por `cdfun`.
- **Pré-condição:** o caixa da **mesma unidade** no **último dia útil da competência** está `CONFIRMADO` (**RN-CXA-009**). Último dia útil = último `YYYY-MM-DD` do mês com peso **> 0** no calendário da unidade (**RN-CAL-001**): seg–sex = 1; sábado só se `sabadoDiaUtil` (0,45 ainda libera o gate); domingo/feriado = 0. **Mensagem:** `Não é possível fechar a visitação: o caixa de {unidade} em {data} ainda não está confirmado.`
- **Pré-condição — fórmulas:** pagas do período (DTEFE, excluídas cortesia `C` e tipo `N` sem fórmula) precisam ter `caixa_requisicao_formula` (RN-VIS-008 / RN-CXA-003). Se menos da metade tiver série, o Fechar **recusa** — senão o retrato congela crédito legado (série `0`). **Mensagem:** complemento de fórmulas incompleto; atualizar as vendas do período e só então fechar.
- **Estados:** competência **aberta** (sem linha em `visitacao_fechamento` ou após reabrir) calcula **ao vivo** o recorte PDF (RN-VIS-008) + projeção (RN-VIS-010). **Fechada:** a tela e a impressão oficial leem **somente o retrato**; sync do painel (`FC04200`) e reimport de caixa **não** alteram o mês fechado.
- **Retrato no fechar (não recalcular depois):** congela o **mesmo recorte da tela aberta** (paga + série = PDF analítico), para consulta, **avaliação posterior** e BI (evolução de representante/prescritor, ranking, queda/oscilação). Inclui: carteira da unidade (`painel_medicos_representantes` naquele instante); **todas** as linhas de médico da grade (unique `fechamento + CRM + UF + unidade do movimento`, com quantidades, `unidadeCarteira` e indicação); totais por representante com **recebido oficial** (soma das unidades de comissão), rejeitado, representatividade, meta, % meta, faixa, comissão, **quantidades** e **breakdown por filial** (`visitacao_fechamento_representante_unidade` + `unidades_comissao`); totais da loja e **blocos de outras filiais** do card TOTAL (`visitacao_fechamento_outra_unidade`). **Não** grava projeção (competência fechada não projeta). **Não** grava o detalhe cupom a cupom (o modal Detalhe **não congela**: continua lendo pagas/fórmulas **ao vivo**). **Não** usar só `painel_medicos_representantes_historico` (RN-REP-002 não registra entrada). Competência já fechada **antes** desta regra precisa **reabrir e fechar de novo** para ganhar o breakdown.
- **Reabrir:** apaga o retrato da competência; volta ao cálculo ao vivo; exige **`visitacao-fechamento:reabrir`**. Trilha de auditoria HTTP. Fechar exige **`visitacao-fechamento:fechar`**. Ver o status na tela usa **`visitacao-acompanhamento:read`**. Novas permissões só no catálogo; atribuição **manual** em Perfis.
- Persistência: `visitacao_fechamento` (unique `unidade + ano + mes`), `visitacao_fechamento_carteira`, `visitacao_fechamento_representante`, `visitacao_fechamento_representante_unidade`, `visitacao_fechamento_outra_unidade`, `visitacao_fechamento_medico`.
...
- **Cards de vendedor — Manipulados (requisição):** teto `valor_formulas` agregado, parcelamento pelo item do cupom, exclusão cortesia/tipo N sem fórmula. **Não** parte por `SERIER` (o vendedor é da paga, não da série). Universo = `caixa_itens_erp` (`tipo_item = REQUISICAO`); atribuição ao vendedor por `caixa_requisicoes_pagas.codigo_vendedor`. Sem paga/`codigo_vendedor`, o valor **não** gera card de vendedor. **Não** segue o recorte PDF da visitação (RN-VIS-008: paga + série, sem corte de item).
- **Cards de vendedor — Marca própria (produtos):** `caixa_itens_erp` com `tipo_item = PRODUTO`; valor = `valor_liquido_item`; atribuição ao vendedor via `codigo_operador_caixa` do pagamento (`caixa_pagamentos_erp`), vinculado a `codigoVendedorErp` do funcionário. Sem operador, o valor **não** gera card.
- **Total da loja (volume / meta da loja):** pagamentos líquidos ERP (`caixa_pagamentos_erp.valor_liquido`) da unidade, eixo **`data_operacao`** (mesmo do movimento/fechamento de caixa), da competência **até o último caixa CONFIRMADO** da unidade (RN-CXA-009). Sem caixa confirmado no mês, o volume é **0**. **Marca própria** = itens `PRODUTO` (`valor_liquido_item`) nos mesmos dias. **Manipulados** = pagamentos − marca própria (o restante do caixa). Não aplica teto de fórmula, cortesia nem tipo N da RN-VIS-008. Esse volume alimenta o card **TOTAL**, o **% da meta da loja** (RN-COM-007) e a trava/incidência **Vendas da loja** (RN-COM-008). Dias ainda **Abertos** (ex. 30 e 31 com último fechamento em 29) **não entram**.
...
- Filtros: **Unidade**, **competência** (mês/ano 2026–2033, com setas para o mês anterior e o próximo, no mesmo padrão da visitação — RN-VIS-009) e **Vendedor**.
- Cards: **TOTAL {unidade}** (ex. `TOTAL INHUMAS`) + **um card por vendedor vinculado** (funcionário da unidade com **`codigoVendedorErp` > 0** no cadastro). Movimento de caixa/orçamento **sem** esse vínculo **não** gera card nem linha. No card da loja, o valor no título (`TOTAL INHUMAS R$ …`) é a **soma de Manipulados + Marca própria** (volume do caixa confirmado, RN-COM-003). Abaixo exibe recebido manipulados, recebido produtos, **Rejeitado** (soma dos orçamentos rejeitados dos vendedores vinculados — RN-COM-003), **meta**, **% meta** e, com competência aberta, **projeção** `(recebido ÷ dias realizados) × dias úteis` e **% da projeção** (calendário **RN-CAL-001** + último caixa CONFIRMADO — **RN-CXA-009**; sábado = 0,45 como **RN-VIS-010**). **Legenda** no filtro: competência (`Agosto 2026`), total de dias úteis e dias realizados. O card da loja usa a **meta da loja** (RN-COM-007); se não cadastrada, soma as metas dos vendedores. Sem meta: **Sem meta** (não 0%). O filtro **Vendedor** restringe apenas os cards e a grade daquele vendedor; o card **Total** permanece o da **unidade na competência** (ano/mês), independente do vendedor selecionado. Nos cards (**Total** e **vendedor**), a faixa superior exibe **Manipulados**, **Marca própria** e **Rejeitado** (valor e quantidade). Clique no **card do vendedor** ou na **linha da grade** abre modal com resumo e a lista de movimentos (manipulados, marca própria e rejeitados) da competência. No cabeçalho do modal: **Imprimir** e **Exportar Excel** do detalhe. O card Total **não** abre modal.
- **Exportar Excel:** botão com ícone após o filtro **Vendedor** (permissão de leitura). Gera planilha da grade visível (vendedores + linha Total), incluindo comissão/bônus só com **`comercial-acompanhamento:comissao`**.
...
- No acompanhamento, o card **Total** usa a meta da loja cadastrada. Se ainda não houver, cai na soma das metas dos vendedores. O **% da meta da loja** e a projeção do Total usam o **volume do caixa confirmado** da unidade (RN-COM-003), não a soma dos cards de vendedor.
...
- **Incidência:** **Vendas próprias** (recebido do vendedor nos manipulados — RN-COM-003) ou **Vendas da loja** (volume do caixa confirmado da unidade naquela base, mesmo do card Total — RN-COM-003).
- **Trava:** campo **Só pagar se a loja atingir (%)**. Vazio = paga sempre. Preenchido (ex.: 100) = zera comissão e bônus daquela base se o % da meta da **loja** (RN-COM-007, volume do caixa confirmado) for menor que o informado.
...
- **Sábado é dia útil:** checkbox **por unidade** em `calendario_unidade.sabadoDiaUtil` (default **false**). Independente da jornada de produção. Na **visitação** e no **comercial** (projeção da RN-VIS-010 / RN-COM-004), sábado marcado vale **0,45 dia útil**; feriado no sábado zera o dia.
...
- **Usuário entrada/saída:** código **`cdusu`** do movimento na `FC12500` (prioritário). **`funcionarioEntrada` / `funcionarioSaida`:** código **`cdfun`** do mesmo movimento (fallback quando `cdusu` ausente ou inválido). Entrada (`cdopera = 01`, aceita `1` com trim): **primeiro** lançamento cronológico entre **todos os `tppcp`**. **`usuarioSaida`:** **primeiro** `02` (`2`); **`funcionarioSaida`:** `cdfun` desse movimento ou, se não houver `02`, **primeiro movimento com `cdopera ≠ 01`**. **`dataSaida` / `horaSaida`:** **primeiro** `02`; se não houver, **primeiro movimento com `cdopera ≠ 01`** na mesma etapa (todos os `tppcp`) — encerramento operacional. Persistência em `producao_etapas_resumo.usuarioEntrada` / `usuarioSaida` e `funcionarioEntrada` / `funcionarioSaida`. **Mesmo segundo:** desempate preferindo `cdusu` maior. Upsert: chave sem `tppcp`; não sobrescreve códigos preenchidos com `null`.
- **Dados históricos:** após deploy, **reimportar** períodos de fechamento (delete + importação manual ou truncar tabela e reimportar); códigos **`cdusu`** (`codigoUsuarioErp`) e **`cdfun`** (`codigoFuncionarioErp`) no cadastro folha são preenchidos **manualmente** (sem conversão automática entre si).
...
- Vínculo ERP: campos **`codigoUsuarioErp`** (`cdusu`) e **`codigoFuncionarioErp`** (`cdfun`) no cadastro de funcionário (`funcionarios`), únicos por unidade; produtividade tenta **`cdusu` primeiro**, depois **`cdfun`**. Preenchimento manual no cadastro folha ou no **modal de etapas** da config de produção (`PUT /producao/config/funcionarios/:id/codigo-usuario-erp`), permissão **`producao-config:update-codigo-usuario-erp`** (independente de editar etapas).
...
- **Resolução do funcionário na linha:** 1) `usuarioSaida` → `funcionarios.codigoUsuarioErp`; 2) se ausente, `funcionarioSaida` → `funcionarios.codigoFuncionarioErp`.
- **Escopo duplo (resumo × funcionários):**
  ...
  - **`unidadesFuncionarios`:** unidades de **cadastro** consideradas na grade, vínculos `producao_funcionario_etapa` e resolução de `codigoUsuarioErp` / `codigoFuncionarioErp` (**por unidade**).
...
- **Caixa local ERP** persiste em `caixa_pagamentos_erp`, `caixa_itens_erp`, `caixa_requisicoes_pagas` e `caixa_requisicao_formula` — **não** na tabela `baixas`.
...
- **Fechamento de Caixa (Atualizar Vendas):** importa **apenas o dia** selecionado na tela (`dataInicio` = `dataFim` = data do caixa), usando a unidade já informada. Não abre modal; exibe mensagem de sucesso ou erro na própria tela, sem resumo/preview da importação. Pagamentos, itens e **requisições pagas** vêm na **mesma** chamada; se o complemento vier incompleto, a API **recusa** a gravação (RN-CXA-003).
...
- Logs de tempo (backend e agente): cada etapa da importação (pagamentos, itens, **requisições pagas**, snapshot, upsert) registra duração em ms e quantidade de linhas. Acima de **15 s** gera `warn` `[lento]` para localizar gargalo (ex. complemento `FC17000` num dia específico).
...
- **Sync do período importado:** após buscar pagamentos, itens e requisições no agente, o backend **remove** de `caixa_pagamentos_erp`, `caixa_itens_erp` e `caixa_requisicoes_pagas` os registros da `(unidade, data)` do segmento cuja `chave_erp` **não** veio no snapshot (ex.: movimento excluído e recriado no ERP com novo cupom/`operid`). Filhas em `caixa_requisicao_formula` saem por **CASCADE**. Em seguida faz upsert das linhas retornadas. **Não** altera baixas de terceiro.
- **Proteção do complemento:** se o snapshot de **requisições pagas** vier **vazio** **ou incompleto** frente às requisições do cupom **já presentes em `FC17000`** (menos da metade), a importação **aborta** — **não** grava pagamentos, itens nem pagas. Mensagem: detalhe incompleto do ERP; atualizar vendas de novo antes de fechar. Cupom ainda **sem baixa** (`FLAGBXA` vazio / sem linha em `FC17000`, típico Uberaba no mesmo dia) **não** dispara a trava: essas requisições ainda não têm detalhe pago no ERP. Item `REQUISICAO` sem paga correspondente **não é gravado** (evita órfã no Fechar). Evita o dia ficar com cupom/total conferido e **sem** `caixa_requisicoes_pagas` quando a paga já existe no Firebird. **Fechar o caixa** do dia recusa se existir requisição gravada no cupom sem linha paga.
- **`caixa_requisicoes_pagas`:** orçamento (`NRORC`), qtd/valor de fórmulas e prescritor **agregado** em `FC12100` por requisição (prioriza série `0`, depois outras fórmulas e requisição-fonte `NRRQUFON`) — usado no caixa detalhado e no comercial. Considera apenas `NRORC > 0`. Se o CRM não vier da fórmula, usa `FC17000.PFCRM/UFCRM/NRCRM`. A busca inclui `FC17000.dtefe >= dataInicio AND dtefe < dataFim+1 dia` **ou** requisição presente no cupom do dia (`FC31200.dtope BETWEEN`). Recorte exclusivo no `DTEFE` porque o campo pode ser DATE ou TIMESTAMP — `BETWEEN` no mesmo dia só pega meia-noite. Grava `tipo_requisicao` (`TPRQU`), `valor_saldo` (`VRSDO`) e `valor_formulas` = soma `FC12100.PRCOBR` × `LEAST(1, VRLIQ/VRRQU)`. Vendedor: `FC17200` (`TRIM(TPTAR)='R'`) e, se vazio, `FC17000.CDFUN`. **Não altera** totais de `caixa_pagamentos_erp`.
- **`caixa_requisicao_formula`:** uma linha por **paga + `SERIER`** (`FC12100`): CRM/UF/nome da série, `NRORC`, `valor_prcobr` e `valor_rateado` (mesmo rateio `VRLIQ/VRRQU` da paga). Unique `(requisicao_paga_id, serie)`. Substitui as filhas a cada upsert da paga. Fonte do crédito do prescritor na visitação (**RN-VIS-008**). **Não** usar `producao_etapas_resumo` para comissão (eixo PCP, fallback de CRM para série 0). No agente (Firebird 2.5), o SQL de pagas continua sem CTE/`IN` correlacionado pesado; as séries saem do mesmo conjunto de `NRRQU` do dia.
...
- **Pré-condição ao confirmar:** não pode haver item `REQUISICAO` **gravado** do dia sem `caixa_requisicoes_pagas`. Cupom ainda sem paga no ERP não entra no snapshot (RN-CXA-003). Se o detalhe baixado não veio, **Atualizar Vendas** de novo, não fechar.
