...
- **Pré-condição:** o caixa da **mesma unidade** no **último dia útil da competência** está `CONFIRMADO` (**RN-CXA-009**). Último dia útil = último `YYYY-MM-DD` do mês com peso **> 0** no calendário da unidade (**RN-CAL-001**): seg–sex = 1; sábado só se `sabadoDiaUtil` (0,5 ainda libera o gate); domingo/feriado = 0. **Mensagem:** `Não é possível fechar a visitação: o caixa de {unidade} em {data} ainda não está confirmado.`
