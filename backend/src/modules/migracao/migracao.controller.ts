...
export class MigracaoController {
  ...
  @ApiOperation({
    ...
    description:
      'Executa a migration para adicionar campos de relacionamento na tabela vendas',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    summary:
      'Migrar vendas para relacionamentos com Cliente, Vendedor e Prescritor',
    description:
      'Busca dados nos bancos legados Firebird e cria/vincula os relacionamentos nas vendas',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    summary:
      'Migrar todos os cadastros (clientes e prescritores) das bases legadas',
    description:
      'Importa todos os registros das tabelas legadas de clientes e prescritores. Vendedores são migrados apenas durante a migração de vendas.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Converte todos os nomes (cliente, vendedor, prescritor) para maiúsculas e normaliza espaços. Ex: "Abnil C. L  Neto" -> "ABNIL C. L NETO"',
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      properties: {
        vendasAtualizadas: {
          type: 'number',
          description: 'Número de vendas atualizadas',
        },
  ...
  @ApiOperation({
    summary:
      'Remover campos texto (cliente, vendedor, prescritor) da tabela vendas',
    description:
      '⚠️ ATENÇÃO: Esta migration remove permanentemente os campos texto (cliente, vendedor, prescritor) da tabela vendas. Execute apenas após validar que todos os dados foram migrados para os relacionamentos (clienteId, vendedorId, prescritorId). A migration inclui verificações de segurança que bloqueiam a execução se houver vendas sem relacionamento válido. FAÇA BACKUP DO BANCO ANTES!',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 400,
    description:
      'Erro: Existem vendas sem relacionamento. Execute primeiro a migração de dados (POST /api/migracao/vendas-relacoes)',
  })
